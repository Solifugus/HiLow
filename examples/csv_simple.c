...
int32_t test_split_join();
int32_t test_stack();
int32_t main();

int32_t test_split_join() {
    char* text = "one,two,three,four";
    DynamicArray* parts = str_split(text, ",");
    printf("Split into %d parts\n", parts->length);
    char* rejoined = array_join_string(parts, "-");
    printf("Rejoined: %s\n", rejoined);
    return parts->length;
}

int32_t test_stack() {
    DynamicArray* numbers = array_new(sizeof(int32_t));
    array_push_i32(numbers, 100);
    array_push_i32(numbers, 200);
    array_push_i32(numbers, 300);
    printf("Pushed 3 numbers, length: %d\n", numbers->length);
    int32_t a = array_pop_i32(numbers);
    int32_t b = array_pop_i32(numbers);
...
int32_t main() {
    printf("%s\n", "=== Split/Join Test ===");
    int32_t parts = test_split_join();
    printf("%s\n", ");\n    print(\"=== Stack Test ===\");\n    let stack_sum: i32 = test_stack();\n\n    print(");
    printf("Total: %d + %d = %d\n", parts, stack_sum, (parts + stack_sum));
    return (parts + stack_sum);
}
