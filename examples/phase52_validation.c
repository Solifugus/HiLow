...
int32_t main() {
    ...
    array_push_i32(numbers, 10);
    array_push_i32(numbers, 20);
    array_push_i32(numbers, 30);
    printf("After push: length = %d\n", numbers->length);
    int32_t popped = array_pop_i32(numbers);
    printf("Popped: %d\n", popped);
    printf("After pop: length = %d\n", numbers->length);
    printf("%s\n", ");\n    print(\"=== String Split/Join ===\");\n    let text: string = \"alpha,beta,gamma,delta\";\n    let words: [string] = text.split(\",\");\n\n    print(f\"Split \'{text}\' into {words.length} parts\");\n\n    let joined: string = words.join(\" - \");\n    print(f\"Rejoined: {joined}\");\n\n    print(");
    printf("%s\n", "=== Combined Test ===");
    char* csv = "red,green,blue";
    DynamicArray* colors = str_split(csv, ",");
    char* formatted = array_join_string(colors, " AND ");
    printf("Colors: %s\n", formatted);
    int32_t total = ((numbers->length + strlen(words)) + colors->length);
    printf("Total arrays length: %d\n", total);
    return total;
}
