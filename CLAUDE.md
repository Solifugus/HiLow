...
**Current phase: 5c — `shared` scalars: a `shared let` scalar is atomically accessed and cross-context watchable (the `docs/hilow-design.md` keyword-unification edit lands here, R4); shared containers and `(deep)` across `shared` are explicitly rejected; resolves deviation 5a-ii (the inbox entry carries the shared scalar's typed release, so a solely-owned cross-thread cell frees correctly at drain). Gate: single-threaded corpus byte-identical; threaded fixtures valgrind-clean. (Phase 5b landed: minimal `async` — an `async { ... }` block compiles to a `void* fn(void*)` pthread entry spawned via `hl_async_spawn` and joined at program exit; outer references captured into a heap env (the boxing analysis gained an async capture boundary); mutating a watched cell from the spawned thread routes through `hl_cell_notify` → the declaring thread's inbox; same-thread firing stays synchronous (R1). Threaded runtime mode (engaged iff the program uses `async`): atomic refcounts + `_Atomic` leak counters via `-DHILOW_THREADED`, and a codegen-emitted loop-backedge safe point (brief §5b amendment) — all gated on `uses_async`, so the single-threaded corpus is byte-identical (verified vs 1c3bd2f). Program exit joins all async threads then main drains last. The re-entrancy `#[ignore]` test activated (ignored 2 → 1; renamed `watcher_reentrant_sync`); five order-insensitive cross-thread fixtures added, valgrind-clean. See docs/STATUS.md.)**
