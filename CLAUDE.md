...
**Current phase: 6 — re-scoping (chat brief in progress). Phase 5 (notification queues + concurrency) is COMPLETE. Standalone 6a ("sendable check, structurally like the write-list check") has DISSOLVED (owner ruling 2026-07-23): the spec's process model stands — no process-spawn construct; processes are separately-launched programs and `shared` is the only cross-process channel — so under today's type universe the sendable check ("what may be declared `shared`") already exists as Phase 5c's scope fence. The referenced write-list check was never built (real 5b = minimal `async`; audit §5b now DESCOPED, §6 SUPERSEDED, `concurrency-design.md` HISTORICAL). Do NOT implement anything from cell-migration-audit §6 until the re-brief lands; shared-in-6a and boundary-locality are parked. Last landed: a docs-+-test-infra-only maintenance commit (ledger repair; valgrind gate lane-split — `valgrind_gate_single_threaded` + `valgrind_gate_threaded`, the threaded lane on `--fair-sched=yes` with reduced spin-N via `HILOW_GATE_SPIN_N`; doc-truth sweep). See docs/STATUS.md. (Phase 5c landed, closing Phase 5: `shared let` i32 scalars — refcounted + atomic + cross-context watchable. The cell gains `sub_lock` (non-NULL ⟺ shared): the payload is accessed atomically (`hl_scalar_get_i32`/`hl_cell_set_i32` branch on it; plain `x += 1` is two atomic ops, a racy RMW = future prover warning), and the subscriber list is guarded — `hl_cell_notify` snapshots under the lock then fires unlocked; add/remove take the lock; non-shared cells take no lock, fast path unchanged. Threaded mode is engaged by `async` OR `shared` (`threaded_mode()`). Deviation 5a-ii graduated: `HiLowCell` gains a `kind` tag + `hl_cell_release_full` dispatches the typed teardown at refcount 0, so a cross-thread last-release of a shared cell frees the container from any thread (C-harness-proven). Spec R4: `shared` unified to one meaning in `docs/hilow-design.md`. Scope fence: shared containers and `(deep)` across `shared` rejected (REJECTION_FIXTURES +2 = 28). Five order-insensitive shared fixtures + the 5a-ii C-harness cases, all valgrind-clean; single-threaded corpus byte-identical vs bc51de4. See docs/STATUS.md.)**
