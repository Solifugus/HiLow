...
**Current phase: 6b — process tier / delivery (GOVERNED BY `docs/phase6-brief.md` §3-6b, rulings R-A–R-E — NOT audit §6, which is SUPERSEDED). Phase 5 COMPLETE; Phase 6a COMPLETE (2026-07-23). 6a landed cross-process `shared` by PLACEMENT: surface `shared("name") let x: i32 = 0`, versioned-header POSIX shm segments (create-or-attach per R-D — creator inits + release-publishes init-complete; attacher bounded-waits + verifies every header field + observes/fires-nothing; persistent, 0600), placeability rejection diagnostics (the "sendable check", not a phase), `hl_scalar_new_i32_placed` + `-lrt`, a two-process valgrind harness, and a C-fork unit harness for the paths HiLow can't reach. Writes already bump the segment epoch (release); NOTHING pulls it yet — that is 6b. 6b scope: the declaring thread tracks (segment, last_seen epoch, last_delivered value) per watched cross-process cell and, at the EXISTING drain safe points (runtime entries + loop back-edges — no new safe points), acquire-loads the epoch; on advance, delivers locally via the 5c in-process path. Fire semantics (adjudicated, §3-6b): `(assigned)` fires iff epoch advanced (at-least-once, coalescible per R3 — fire count ≠ remote-assignment count, fixtures must not assert it); `(changed)` fires iff drain-time value ≠ last-DELIVERED value (an ABA inside one coalescing window fires nothing). Backoff polling at idle safe points (yield→bounded sleep, cap ~1ms); blocking futex wait is deferred. The spec's Cross-Process Watchers example (design:1765–1791) becomes a real two-program fixture with a one-token edit, and the spec text gets that edit in the SAME commit. Existing corpus byte-identical; cross-process fixtures run under the two-process harness with both processes valgrind-clean. Do NOT implement 6c (lifecycle — mini-brief after 6b), shared containers, or multi-var segments. See docs/phase6-brief.md §3-6b + state-of-migration §I.3. (Phase 5c landed, closing Phase 5: `shared let` i32 scalars — refcounted + atomic + cross-context watchable. The cell gains `sub_lock` (non-NULL ⟺ shared): the payload is accessed atomically (`hl_scalar_get_i32`/`hl_cell_set_i32` branch on it; plain `x += 1` is two atomic ops, a racy RMW = future prover warning), and the subscriber list is guarded — `hl_cell_notify` snapshots under the lock then fires unlocked; add/remove take the lock; non-shared cells take no lock, fast path unchanged. Threaded mode is engaged by `async` OR `shared` (`threaded_mode()`). Deviation 5a-ii graduated: `HiLowCell` gains a `kind` tag + `hl_cell_release_full` dispatches the typed teardown at refcount 0, so a cross-thread last-release of a shared cell frees the container from any thread (C-harness-proven). Spec R4: `shared` unified to one meaning in `docs/hilow-design.md`. Scope fence: shared containers and `(deep)` across `shared` rejected (REJECTION_FIXTURES +2 = 28). Five order-insensitive shared fixtures + the 5a-ii C-harness cases, all valgrind-clean; single-threaded corpus byte-identical vs bc51de4. See docs/STATUS.md.)**
