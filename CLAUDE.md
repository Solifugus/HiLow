...
**Current phase: NONE — THE CELL MIGRATION IS COMPLETE (6c landed 2026-07-23, closing Phase 6). AWAITING OWNER DIRECTION; DO NOT START NEW FEATURE WORK.** Phase 6 is the last phase in `docs/cell-migration-audit.md`, so the audit has **no successor** — "advance to the next audit phase" has no referent, and picking a `docs/development-plan.md` phase is an owner decision, not one to infer. The single concrete outstanding pin is the sole remaining `#[ignore]` (unknown-with-options), destined for development-plan **Phase 9b (Time Type)** — that is the indicated candidate, not a ruling. Whatever phase is chosen, its plan section is a PREMISE TO CHECK, not a spec: those sections predate the migration, and every migration phase from 4a onward found at least one stale premise (the standing caveat — premise-check against the tree and report staleness BEFORE implementing). **Two items are open for owner adjudication, both raised in the 6c debrief:** (1) **the spec's `for`-loop divergence** — the spec documents C-style `for (let i = 0; i < 10; i += 1)` (10 occurrences) and the single-variable iterator form `for (let item in array)` (8 occurrences) as language features, but the parser implements ONLY the tuple form `for (let (i, v) in expr)` (5 occurrences), so 18 of the spec's 23 `for` loops do not compile; the 6c mini-brief's prescribed remedy ("convert C-style to iterator form") is void because its target form is itself unimplemented, so the sweep was NOT performed — resolving this requires deciding whether the parser is behind the spec or the spec over-promises. (2) the 6b `for`→`while` edit in the Cross-Process Watchers example (state-of-migration §J.2 6b-v). Phase 6 CLOSURE — what the process tier guarantees and does not: state-of-migration §K.3. 6c added NO compiler or runtime code; it pinned crash robustness (SIGKILL producer mid-write / SIGKILL consumer / full population turnover — all passed unchanged), documented segment cleanup (ruling c2, docs-only) and watcher-body idempotency in the spec, and recorded the liveness (c3) and shared-container (c4) deferrals in `docs/phase6-brief.md` §6. GOVERNED BY `docs/phase6-brief.md` (rulings R-A–R-E + 6c's c1–c4) — NOT audit §6 (SUPERSEDED). Historical detail on the completed sub-phases follows.

**(Historical — 6c prior state) 6c — process tier / lifecycle. NEEDS ITS MINI-BRIEF FIRST (per R-E: 6c is only a SKETCH in `docs/phase6-brief.md` §3-6c; the owner writes the mini-brief after seeing 6b's reality — the 4a lesson). Do NOT implement 6c from the sketch until that mini-brief lands. Phase 5, 6a, 6b all COMPLETE (2026-07-23). 6c expected scope (sketch, subject to the mini-brief): segment cleanup policy (persistent-by-default vs unlink-on-last-detach + crash tolerance), attach/detach visibility, and process liveness as a WATCHABLE (pid-as-cell — supervision becomes ordinary watcher code). GOVERNED BY `docs/phase6-brief.md` (rulings R-A–R-E) — NOT audit §6 (SUPERSEDED). 6b landed cross-process DELIVERY: the declaring thread keeps one record per watched placed cell `{cell, last_seen_epoch, last_delivered}` and pulls the segment epoch at existing safe points (runtime entries + loop back-edges); on advance it delivers via the 5c notify path. The pull unifies cross-thread-local + cross-process (placed cross-thread notify skips the inbox); a same-thread write fires synchronously (R1) and advances the record so the pull skips it (dedup). `(assigned)` on epoch advance (coalescible — fire count is NEVER asserted); `(changed)` iff drain-time value ≠ last-delivered. Idle backoff (yield→sleep ≤1ms); residual delivery at final drain. The spec's Cross-Process Watchers example is a live 2-program fixture. FLAGGED spec deviation (6b): its producer's C-style `for` was changed to `while` because HiLow's `for` is iterator-form only (the example never compiled — pre-existing, independent of the named-shared edit); see state-of-migration §J.2 6b-v — owner review invited. NOT in scope until adjudicated/re-briefed: shared containers, multi-var segments, blocking futex wait, cross-machine. See docs/phase6-brief.md §3-6c + state-of-migration §J.3. 6a landed cross-process `shared` by PLACEMENT (surface `shared("name") let x: i32 = 0`, versioned-header POSIX shm segments, placeability rejection diagnostics = the "sendable check", `hl_scalar_new_i32_placed` + `-lrt`, two-process + C-fork harnesses). (Phase 5c landed, closing Phase 5: `shared let` i32 scalars — refcounted + atomic + cross-context watchable. The cell gains `sub_lock` (non-NULL ⟺ shared): the payload is accessed atomically (`hl_scalar_get_i32`/`hl_cell_set_i32` branch on it; plain `x += 1` is two atomic ops, a racy RMW = future prover warning), and the subscriber list is guarded — `hl_cell_notify` snapshots under the lock then fires unlocked; add/remove take the lock; non-shared cells take no lock, fast path unchanged. Threaded mode is engaged by `async` OR `shared` (`threaded_mode()`). Deviation 5a-ii graduated: `HiLowCell` gains a `kind` tag + `hl_cell_release_full` dispatches the typed teardown at refcount 0, so a cross-thread last-release of a shared cell frees the container from any thread (C-harness-proven). Spec R4: `shared` unified to one meaning in `docs/hilow-design.md`. Scope fence: shared containers and `(deep)` across `shared` rejected (REJECTION_FIXTURES +2 = 28). Five order-insensitive shared fixtures + the 5a-ii C-harness cases, all valgrind-clean; single-threaded corpus byte-identical vs bc51de4. See docs/STATUS.md.)**
...
3. **`docs/cell-redesign-brief.md`** and **`docs/cell-migration-audit.md`** — the
   ...
   **Exception — Phase 6:** the audit's §Phase 6 is SUPERSEDED (it was written
   against a counterfactual 5b + a never-built write-list check). Phase 6 is
   governed instead by **`docs/phase6-brief.md`** (ratified 2026-07-23, rulings
   R-A–R-E): cross-process `shared` by placement in named shm segments. For any
   Phase-6 work, that brief is the source of truth; do not read the audit's §6
   as a plan.

If these documents disagree with each other, stop and ask — do not pick one silently.
