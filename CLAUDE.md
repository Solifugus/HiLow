...
**Current phase: 6a — process tier / placement (GOVERNED BY `docs/phase6-brief.md`, ratified 2026-07-23 — NOT audit §6, which is SUPERSEDED). Phase 5 (notification queues + concurrency) is COMPLETE. Cross-process `shared` is by PLACEMENT in named POSIX shm segments (rulings R-A–R-E), not transport/serialization (retired concepts). Surface: `shared("name") let x: i32 = 0` (bare `shared` keeps its 5c cross-thread meaning). The "sendable check" lands as three placeability rejection diagnostics (non-scalar `shared("name")` = not placeable), not a phase. 6a scope: grammar+AST for `shared("name")`, name validation, the placeability fence, one-var-per-segment versioned header, create/attach per R-D (first creator inits; attachers verify header + observe, fire nothing; bounded init-wait), payload storage moved to the mapped slot (5c cell machinery otherwise intact), write bumps epoch (release) even though nothing pulls until 6b, detach at exit (segments persist by default), and the TWO-PROCESS test harness (both processes under valgrind, harness unlinks its segments). 6a opening commit DONE (2026-07-23): test_phase7a_integration deleted (ignored ledger 2 → 1; `hl_object_get_str` coverage retained by test_object_mixed_types_integration, no replacement fixture needed). 6a PHASE WORK (grammar/AST for `shared("name")`, placeability fence, shm header + create/attach, two-process harness) is the NEXT session's job. Existing corpus byte-identical (new syntax is the only entry point — any diff is stop-and-report). Do NOT implement 6b (delivery) or 6c (lifecycle) this phase. See docs/phase6-brief.md §3-6a. (Phase 5c landed, closing Phase 5: `shared let` i32 scalars — refcounted + atomic + cross-context watchable. The cell gains `sub_lock` (non-NULL ⟺ shared): the payload is accessed atomically (`hl_scalar_get_i32`/`hl_cell_set_i32` branch on it; plain `x += 1` is two atomic ops, a racy RMW = future prover warning), and the subscriber list is guarded — `hl_cell_notify` snapshots under the lock then fires unlocked; add/remove take the lock; non-shared cells take no lock, fast path unchanged. Threaded mode is engaged by `async` OR `shared` (`threaded_mode()`). Deviation 5a-ii graduated: `HiLowCell` gains a `kind` tag + `hl_cell_release_full` dispatches the typed teardown at refcount 0, so a cross-thread last-release of a shared cell frees the container from any thread (C-harness-proven). Spec R4: `shared` unified to one meaning in `docs/hilow-design.md`. Scope fence: shared containers and `(deep)` across `shared` rejected (REJECTION_FIXTURES +2 = 28). Five order-insensitive shared fixtures + the 5a-ii C-harness cases, all valgrind-clean; single-threaded corpus byte-identical vs bc51de4. See docs/STATUS.md.)**
