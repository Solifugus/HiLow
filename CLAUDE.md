...
**Current phase: 5b — minimal `async`: an `async { ... }` block compiled to a C function spawned on a pthread (no prover integration), so the Phase-5a inbox is reachable from HiLow source; first cross-thread fixtures (order-insensitive invariants only); the re-entrancy `#[ignore]` test activates (ignored 2 → 1); threaded runtime mode (atomic refcounts iff the program uses `async`/`shared`) so the single-threaded corpus stays byte-identical. Gate: single-threaded corpus byte-identical; threaded fixtures valgrind-clean. (Phase 5a landed: the notification-queue infrastructure — four statics migrated to `_Thread_local`, a per-thread `HiLowThreadContext` with an MPSC mutex-guarded inbox + registry, watcher `owner_ctx` + routing in `hl_cell_notify`, safe-point drain hooks at allocation/syscall boundaries, and C-level inbox unit tests. Single-threaded corpus byte-identical; program-logic codegen untouched; valgrind gate green. See docs/STATUS.md.)**
