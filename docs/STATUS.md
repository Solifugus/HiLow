...
Phase: Cell migration Phase 1.5e COMPLETE (weak-after-death semantics, audit §5 item 6)
Status: Reading a weak property whose referent has died yields `unknown` with reason "weak referent released"; member access on it propagates per the spec's unknown rules (no fatal exit). Implemented by typing weak property reads as `T?` riding the existing Phase 9 unknown ecosystem: `hl_object_get_weak` returns a fresh optional (retained referent while alive / unknown after death), `hl_optional_member_{i32,str,object}` propagate through member access (the SAME unknown instance), HiLowOptional gained an object payload kind. hilow-design.md edited in the same commit: new "Weak References" subsection (no retain on store, slot nulled on referent death, dead read yields unknown with the stated reason, member access propagates) and the line-564 dynamic-removal claim amended per the append-only-indices ruling. test_weak_after_death_unknown_integration is LIVE; weak_after_death_unknown.hl removed from REJECTION_FIXTURES and is valgrind-clean under the gate.
Branch: main
Last commit: Phase 1.5e: weak-after-death semantics

Tests (2026-07-15, main, --no-fail-fast): ALL suites green — integration 260 passed / 0 failed / 6 ignored (all 6 documented below); valgrind gate 1 passed; 110 lexer, 68 parser, 62 typecheck, 27 typecheck_module, 24 codegen, 12 object_typecheck, 8 resolver, 6 object_parser, plus smaller suites, all 0 failed.

Cell redesign: Phases 1, 1.5a, 1.5b, 1.5c, 1.5d, 1.5e complete — Phase 1.5 pre-migration hardening is DONE. Next: Phase 2a — cell header onto HiLowArray (mechanical layout change, firing behavior identical).
...
All in tests/integration_tests.rs; each ignore string names its bug/phase. As of 2026-07-15 (Phase 1.5e) there are 6:
...
- `test_watcher_multi_array_capture_unregister_integration` — audit §3.4(b), multi-array env unregisters from only the last array; UAF. Flips in Phase 2b. Program on KNOWN_MEMORY_BUGS.
- `test_watcher_null_key_scope_death_integration` — audit §3.4(c), "NULL"-keyed no-capture registrations never unregistered; fires after scope death with invalid reads. Flips in Phase 2b. Program on KNOWN_MEMORY_BUGS.
- `test_watcher_temp_env_statement_integration` — audit §3.4(d) territory; statement-temporary watcher expr (call argument) never registers and leaks. Flips in Phase 2b. Program on KNOWN_MEMORY_BUGS.

(`test_weak_after_death_unknown_integration` flipped live in Phase 1.5e, 2026-07-15.)
