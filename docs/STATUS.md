...
Phase: Cell migration Phase 1.5a COMPLETE (char*/HiLowArray* representation-split failures fixed — full suite green)
Status: All 13 representation-split integration failures fixed; suite fully green (239 integration passed / 0 failed / 1 documented ignore). Six root causes fixed: generate_statement re-entrancy (temp decls/cleanup stolen by nested statement generation), return-path temp cleanup emitted after `return` (dead code → leaks), strcmp on HiLowArray* in match/switch string comparisons + match-as-expression result ownership, for-in object keys/values on char* paths, hl_unknown_get_reason returning char*, and closure env capturing string params without retain (use-after-free).
Branch: main
Last commit: Phase 1.5a: fix char*/HiLowArray* representation-split failures (13 tests green)

Tests (2026-07-14, main, --no-fail-fast): ALL suites green — integration 239 passed / 0 failed / 1 ignored; 110 lexer, 68 parser, 62 typecheck, 27 typecheck_module, 24 codegen, 12 object_typecheck, 8 resolver, 6 object_parser, plus smaller suites, all 0 failed.

Cell redesign: Phase 1 (audit) and Phase 1.5a (representation-split fix) complete. Next: Phase 1.5b — valgrind/ASan gating on the lifetime/no-leak tests (audit §4.4 item 10), then 1.5c gap tests, then arrays-first migration.
...
- **Temps of an enclosing statement leak on `return` from inside a match/switch arm.** Phase 1.5a made `generate_statement` re-entrant (each statement releases exactly its own temps), and `return` now releases the return statement's own temps. But a `return` nested inside a match/switch arm cannot see the *enclosing* statement's temps (e.g. a string match subject) — those release at the enclosing statement's end, which a taken return skips. No test exercises this shape; before 1.5a those temps were never released on any path. Proper fix arrives with migration Phase 4 (per-statement release lists).

- **For-in object key leaks on `break`/`continue` inside the loop body.** The key is now a managed string released at the bottom of each iteration and heap_owners-tracked during body generation (so `return` inside the body releases it), but `break`/`continue` cleanup only covers the body's own scope depth. No test exercises break/continue in object for-in. Same Phase 4 machinery is the proper fix.
