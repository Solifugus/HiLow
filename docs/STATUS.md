...
Phase: Cell migration Phase 1.5d COMPLETE (gap tests — no compiler/runtime code changes)
Status: All audit §4.4 pre-migration gap tests exist. Live pins (current correct behavior): early-return watcher lifetime (scalar + array), shadowing + array-watcher identity, insert firing order (CHANGED then ADDED), array-watcher factory, stealth-return rejection (fixture finally wired), string-watcher compile diagnostic (per §5 item 2). Six #[ignore]d expected-behavior tests document adjudicated future semantics: re-entrant mutation asserts deferred queue firing (Phase 5), the four §3.4 env-keying latent bugs (flip in Phase 2; their programs are on the gate's KNOWN_MEMORY_BUGS with citations), and weak-after-death → unknown (Phase 1.5e; its program is on REJECTION_FIXTURES since it cannot compile before 1.5e). Weak-after-death semantics ADJUDICATED (audit §5 item 6). Object property removal verified nonexistent — constraint recorded next to the WeakRef re-keying note in the audit.
Branch: main
Last commit: Phase 1.5d: gap tests

Tests (2026-07-15, main, --no-fail-fast): ALL suites green — integration 259 passed / 0 failed / 7 ignored (all 7 documented below); valgrind gate 1 passed (KNOWN_MEMORY_BUGS carries the four adjudicated §3.4 latent-bug programs; REJECTION_FIXTURES gained string_watcher_rejected.hl and weak_after_death_unknown.hl); 110 lexer, 68 parser, 62 typecheck, 27 typecheck_module, 24 codegen, 12 object_typecheck, 8 resolver, 6 object_parser, plus smaller suites, all 0 failed.

Cell redesign: Phases 1, 1.5a, 1.5b, 1.5c, 1.5d complete. Next: Phase 1.5e — weak-after-death implementation (dead-weak read → unknown "weak referent released", spec edit in the same commit), then Phase 2 arrays-first migration.
...
*(Resolved 2026-07-15: "Weak reference access after referent death" — ADJUDICATED, recorded as audit §5 item 6: a dead-weak read yields `unknown` with reason "weak referent released"; member access on it propagates per the spec's unknown rules. Implementation is Phase 1.5e with the hilow-design.md spec edit in the same commit. Expected-behavior test exists as #[ignore]d test_weak_after_death_unknown_integration; its program is on the gate's REJECTION_FIXTURES until 1.5e.)*
...
All in tests/integration_tests.rs; each ignore string names its bug/phase. As of 2026-07-15 (Phase 1.5d) there are 7:
...
- `test_weak_after_death_unknown_integration` — adjudicated weak-after-death semantics (audit §5 item 6). Flips in Phase 1.5e; program on REJECTION_FIXTURES until then.
