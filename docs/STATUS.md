...
Phase: Cell migration Phase 1.5c COMPLETE (object ownership discipline — KNOWN_MEMORY_BUGS empty, full suite green including the gate)
Status: The object double-release class is fixed under one axiom: every store of a heap reference (proto link, object property, array element, local binding) retains; every overwrite, removal, and owner death releases exactly once; weak references neither retain nor release. All 17 KNOWN_MEMORY_BUGS programs are valgrind-clean and the list is EMPTY (the gate now enforces cleanliness for them like every other program). Weak references are actually implemented now (is_weak was never set anywhere before; weak properties were strong-released AND unretained). Six new pinning tests cover the double-release-prone store patterns.
Branch: main
Last commit: Phase 1.5c: object ownership discipline

Tests (2026-07-15, main, --no-fail-fast): ALL suites green — integration 252 passed / 0 failed / 1 ignored; valgrind gate 1 passed with KNOWN_MEMORY_BUGS empty; 110 lexer, 68 parser, 62 typecheck, 27 typecheck_module, 24 codegen, 12 object_typecheck, 8 resolver, 6 object_parser, plus smaller suites, all 0 failed.

Cell redesign: Phases 1, 1.5a, 1.5b, 1.5c complete. Next: Phase 1.5d — gap tests (audit §4.4 items 1, 4, 5, 6, 7, 9, pinning current behavior; latent-bug expected-fail/ignored tests per §5 item 4), then Phase 2 arrays-first migration.
...
**Weak reference access after referent death — spec is silent (surfaced by Phase 1.5c, needs adjudication, suggested home: 1.5d)**: When a weak target dies, the holder's slot is nulled (runtime WeakRef machinery). A subsequent read of the weak property returns NULL, and member access on it (`holder.ref.name`) falls through the getter's proto walk to `property 'name' not found` → `exit(1)`. The spec mentions weak only as "cycle detection is opt-in via `weak`" (hilow-design.md). Current behavior is reported here, not pinned by a test; decide the intended semantics (runtime error with a weak-specific message? nothing-propagation? require a narrowing check?) before pinning.
