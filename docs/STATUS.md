...
Phase: Cell migration Phase 1.5b COMPLETE (control-transfer temp cleanup + valgrind gate — full suite green including the gate)
Status: The two 1.5a-disclosed leaks are fixed (return past match/switch-arm temps; for-in key on break/continue) with four pinning tests; a valgrind gate now compiles and runs all 245 entry programs under tests/programs/ (tests/valgrind_gate.rs, ~24s, part of the default suite). The gate surfaced two pre-existing bug classes: the string-switch break double-release (fixed this session) and object double-release via unretained second references (17 programs — adjudicated to new Phase 1.5c, tracked in the gate's KNOWN_MEMORY_BUGS list which fails if an entry comes back clean). Three format-specifier fixtures repaired (f-string colon was parsed as type ascription; {n:b} codegen was on char*) and promoted to integration tests.
Branch: main
Last commit: Phase 1.5b: control-transfer temp cleanup + valgrind gate

Tests (2026-07-15, main, --no-fail-fast): ALL suites green — integration 246 passed / 0 failed / 1 ignored; valgrind gate 1 passed; 110 lexer, 68 parser, 62 typecheck, 27 typecheck_module, 24 codegen, 12 object_typecheck, 8 resolver, 6 object_parser, plus smaller suites, all 0 failed.

Cell redesign: Phases 1, 1.5a, 1.5b complete. Next: Phase 1.5c — object ownership discipline (fix the object double-release class, empty the gate's KNOWN_MEMORY_BUGS list), then 1.5d gap tests, then arrays-first migration.
...
- **Object double-release via unretained second reference (fix in Phase 1.5c "object ownership discipline").** An object referenced from two places — a proto link, an array element, or a weak target — is released down both paths; `hl_object_release` then reads/writes the refcount of freed memory (use-after-free, verified by valgrind). Invisible to the emitted alloc/free counter because the garbage refcount usually skips the second free, so the affected integration tests pass. 17 programs exhibit it, enumerated in `KNOWN_MEMORY_BUGS` in `tests/valgrind_gate.rs` (proto_*, is_object_*, array_objects_*, weak_*, method_this_proto, for_in_proto_excluded); the valgrind gate expects them to fail and flags any that come back clean, so the list cannot rot. Adjudicated 2026-07-15 (audit §5 item 5): fixed in the current machinery because retain/release call-site discipline transfers to the cell model.
