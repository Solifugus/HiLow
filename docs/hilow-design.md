...
- Content modifiers in a **declaration-form** watcher (`(added)`, `(removed)`, `(moved)`, `(deep)`) subscribe the value the variable *currently* holds and **follow rebinding**: on assignment, the subscription moves from the old value to the new one — deep watching extends into the new value's nested structure — so the watcher always observes the variable's current contents. Mutations of the previously-held value no longer fire it after rebinding. Retargeting completes before the variable's own `(changed)`/`(assigned)` subscribers fire, and it moves subscriptions, not watcher state: a paused watcher's subscriptions still follow.
